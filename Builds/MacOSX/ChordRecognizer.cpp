...
namespace
{
    ...
    constexpr float kAcceptThreshold       = 0.35f;
    constexpr float kStrongThreshold       = 0.5f;
    constexpr float kMinMarginToSwitch     = 0.00f;
    constexpr int   kFramesToSwitch        = 1;
...
void ChordRecognizer::update(const std::vector<float>& fftMagnitudes)
{
    ...
    {
        for (int i = 0; i < 12; ++i)
            smoothedChroma[i] = (1.0f - kChromaEmaAlpha) * smoothedChroma[i]
                              + kChromaEmaAlpha * frameChroma[i];
    }
...
float ChordRecognizer::cosineSimilarity(const ChromaVector& a, const ChromaVector& b) const
{
